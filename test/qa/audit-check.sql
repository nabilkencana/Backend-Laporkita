WITH candidates AS (
  SELECT id, status, report_code FROM reports
  WHERE (SELECT count(*) FROM report_status_history h WHERE h.report_id = reports.id) >= 3
  ORDER BY random() LIMIT 20
),
hist AS (
  SELECT h.report_id, h.status, h.changed_by,
         row_number() OVER (PARTITION BY h.report_id ORDER BY h.created_at) AS rn,
...
SELECT c.report_code, c.status AS final_status,
       ...
       count(h.changed_by) AS with_changed_by,
       ...
       count(*) FILTER (WHERE h.prev_status IS NOT NULL AND NOT (
         ...
         (h.prev_status='disputed' AND h.status='in_progress'))) AS illegal_transitions
FROM candidates c
